...
`plugin\<插件>\LogFile\infolog*.txt` 里有没有 `Received run ok msg`。
...
- **报告按分层推进,共 10 节** —— 基础事实 → 宿主侧设备层(不走 VBox 的交换机/PC 类设备) →
  host-only 六层(驱动完整性/适配器/绑定/名字比对) → eNSP 本体(性能计数器/防火墙/端口/抓包驱动) →
...
设备起不来的问题仍建议走 [Issues](../../issues) —— 定位依赖 `环境检查.bat` 产出的报告,群里凭文字描述判断不了。
