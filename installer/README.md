...
- **CPU / VT-x** —— BIOS/UEFI 虚拟化是否开启、VMX 扩展是否可用；
- **Hyper-V / WHP / 内存完整性(HVCI) / 虚拟机平台** —— 任一启用都会抢占 VT-x，VBox 7.x 会退到 WHP 后端，eNSP 老镜像在此后端下可能 error 40；命中时附上 `bcdedit /set hypervisorlaunchtype off` 等排查提示；
...
**提示"需要管理员权限"** —— 没经 `安装.bat` 直接跑了 `install.ps1`。请双击 `安装.bat`(它会让 `install.ps1` 提权打补丁、再用你的登录账户注册)。
...
**设备启动报 error 40 / 起不来** —— 先看安装日志(`%TEMP%\ensp-vbox-shim-install.log`)开头的**环境检测**段,或重跑一次 `-Check`。常见成因都在那里标了出来:Hyper-V/WHP/HVCI 抢占 VT-x(日志会给关闭命令)、`VBox\x86\` 缺 x86 VCRT(`0x800700C1`)、BIOS 未开虚拟化、版本伪装未写入。对症处理即可。
