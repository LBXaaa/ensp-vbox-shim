<#
.SYNOPSIS
    ensp-vbox-shim 一键安装编排器 —— 打补丁 + 注册基础设备 VM + 运行时环境检测,只点一次。

.DESCRIPTION
    本脚本【不提权】,以双击它的那个用户(= 平时启动 eNSP 的人)身份运行,分三段:

      安装前检查(只读):先把问题报出来,并在这里征得修复同意。
      第 1 步:把 install.ps1 作为子进程【提权】运行(写 HKLM + Program Files,机器级)。
               UAC 在此弹一次。等它结束并检查退出码。
      第 2 步:仅当第 1 步成功。回到本【非提权】上下文,判定"当前账户是否就是登录用
               eNSP 的那个交互用户"(SID 比对):
                 - 是 -> 直接跑 register_vms.ps1,VM 注册写进正确的 %USERPROFILE%。
                 - 否 -> 跳过注册,提示用户用登录账户双击 注册设备.bat。
      第 3 步:再采一次运行时环境事实(diag.ps1,只读,出报告)。无损档直接修;
               需确认档先明示影响再问。两档都要管理员,所以【确实查出可修项时】才会
               再弹一次 UAC 执行 —— 没有可修项就不弹、也不执行。

    install 必须提权(机器级),register 必须用登录用户令牌(写用户级
    %USERPROFILE%\.VirtualBox\VirtualBox.xml,否则 eNSP 看不到)。两个权限上下文不同,
    所以拆成独立进程跑。检测与修复也是分开的:检测只读不提权,修复提权且只执行
    已征得同意的计划,不自行判断该修什么。
